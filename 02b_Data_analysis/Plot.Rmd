...
discrete_temperate <- temperate %>%
  ggplot(aes(y = Method, 
             yend = Method, 
             x = as.Date("2021-01-01") + start, 
             xend = as.Date("2021-01-01") + end, 
             color = Season)) +
  geom_segment(lwd = 5) +
  theme_bw() +
...
continuous <- continuous_data %>%
  ggplot(aes(x = as.Date("2021-01-01") + yday, y = Value * 100, color = Variable)) +
...
```{r}
jpeg("../03a_Figures/Seasons_data.jpg", width = 9, height = 4, units = "in", res = 300)
