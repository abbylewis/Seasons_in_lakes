...
write_csv(discrete, "../01b_Processed_data/Discrete_seasons.csv", row.names = FALSE)
...
lake_relabel <- function(lakes){
  index <- which(LAKES == lakes)
  labels <- c("`Lake Erken (2018): 59.84 ºN, 18.63 ºE; Z`[max]~`= 21 m`", 
           "`Arendsee (2020): 52.89 N, 11.47 E; Z`[max]~`= 50 m`",
           "`Lake Ägeri (2023): 47.12 ºN, 8.62 ºE; Z`[max]~`= 83 m`", 
           "`Midway Pond (2023): 44.93 ºN, 70.54 ºW; Z`[max]~`= 9 m`", 
           "`Lake Sunapee (2008): 43.38 ºN, 72.05 ºW; Z`[max]~`= 32 m`", 
           "`Mohonk Lake (2017): 41.77 ºN, 74.16 ºW; Z`[max]~`= 18 m`",
           "`Beaverdam Reservoir (2021): 37.31 ºN, 80.18 ºW; Z`[max]~`= 11 m`",
           "`Lake Rerewhakaaitu (2022-2023): 38.29 ºS, 176.5 ºE; Z`[max]~`= 15 m`")
  return(labels[index])
}
...
discrete_ice <- discrete_strat +
  ...
  theme(legend.position = "bottom",
        ...
        strip.clip= "off") +
  guides(color = guide_legend(ncol = 1, title.position="top")) +
  facet_wrap(~Lake, ncol = 1, labeller = as_labeller(lake_relabel, default = label_parsed))
