...
in_lake <- bvr_strat %>% 
  ...
  mutate(Method = "Stratification",
         x = start,
         xend = end,
         Lake = factor(Lake, levels = c("Lake Erken (2018)",
                                        "Lake Sunapee (2008)", 
...
ice_format <- bvr_ice %>%
  ...
  full_join(sun_ice) %>%
  full_join(erken_ice) %>%
  mutate(Lake = factor(Lake, levels = c("Lake Erken (2018)",
                                        "Lake Sunapee (2008)", 
                                        "Beaverdam Reservoir (2021)", 
                                        "Rerewhakaaitu (2022-2023)")))
...
```{r}
jpeg("../03a_Figures/Seasons_data.jpg", width = 9, height = 8, units = "in", res = 300)
ggarrange(discrete_strat, continuous, align = "h")
dev.off()

jpeg("../03a_Figures/Seasons_data_ice.jpg", width = 8.5, height = 7, units = "in", res = 300)
