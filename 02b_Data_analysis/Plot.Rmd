...
in_lake <- bvr_strat %>% 
  ...
  mutate(Method = "Stratification",
         x = start,
         xend = end,
         Lake = factor(Lake, levels = c("Lake Sunapee (2008)", 
                                        "Beaverdam Reservoir (2021)", 
                                        "Rerewhakaaitu (2022-2023)")))

ice_format <- bvr_ice %>%
  full_join(rere_ice) %>%
  full_join(sun_ice) %>%
  mutate(Lake = factor(Lake, levels = c("Lake Sunapee (2008)", 
                                        "Beaverdam Reservoir (2021)", 
                                        "Rerewhakaaitu (2022-2023)")))
...
continuous <- continuous_data %>%
  filter(!Variable %in% c("Bottom-water DO", "Surface turbidity","Surface DO")) %>%
...
```{r}
jpeg("../03a_Figures/Seasons_data.jpg", width = 9, height = 7, units = "in", res = 300)
ggarrange(discrete_strat, continuous, align = "h")
dev.off()

jpeg("../03a_Figures/Seasons_data_ice.jpg", width = 8.5, height = 6, units = "in", res = 300)
...
jpeg("../03a_Figures/Seasons_data_ice2.jpg", width = 10, height = 6, units = "in", res = 300)
