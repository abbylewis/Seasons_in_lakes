...
(gg.clock.mohonk <- ggplot(data = all_mohk) +
  ...
  geom_rect(aes(xmin = (yday - 1) * 2 * pi / 365, xmax = yday * 2 * pi / 365, ymin = y * 0.9, ymax = y * 1, fill = day_length), show.legend = FALSE) +
  scale_fill_gradient(limits = c(9, 16), low = "blue", high = "yellow", breaks = c(9, 12, 15), labels = c(9, 12, 15), name = "Day\nlength (hr)") +
  geom_line(aes(x = yday * 2 * pi / 365, y = y * 0.9), color = "black", size = 0.2) +
  geom_line(aes(x = yday * 2 * pi / 365, y = y * 1), color = "black", size = 0.2) + # inner and outer lines
  ...
  # day length label
  geom_label(data = tibble(x = (c(130)) * 2 * pi / 365, y = y_limit_upper), aes(x = x, y = y), label = "Day length", size = 9 * (5 / 14)) +
  ...
  theme(
    ...
    legend.spacing.y = unit(0.1, "cm"), # decrease the space between the two legends
  ...
  guides(
    ...
    color = guide_legend(order = 1, override.aes = list(size = 3)), # reorder the legends so seasons goes first
    fill = "none" # reorder the legends so stratification goes second
  )
...
# Get out rere data####
rere_cont <- continuous_data2 %>%
  filter(Lake == "Lake Rerewhakaaitu (2022-2023)") %>%
...
rere_strat <- read_csv("01a_Raw_data/Rerewhakaaitu_buoy_wq.csv") %>%
  ...
  mutate(delta_density = ifelse(delta_density < 0, 0, delta_density)) # get rid of negatives
ggplot(data = rere_strat, aes(x = Date, y = delta_density)) +
  geom_point()
...
ggplot(data = rere_daylength, aes(x = yday, y = DayLength)) +
  geom_point()
...
(gg.clock.rere <- ggplot(data = all_rere) +
  ...
  #
  # Rectangle for the daylength as the inner circle####
  geom_rect(aes(xmin = (yday - 1) * 2 * pi / 365, xmax = yday * 2 * pi / 365, ymin = all_mohk$y[1] * 0.9, ymax = all_mohk$y[1] * 1, fill = day_length), show.legend = TRUE) +
  scale_fill_gradient(limits = c(9, 16), low = "blue", high = "yellow", breaks = c(9, 12, 15), labels = c(9, 12, 15), name = "Day\nlength (hr)") +
  geom_line(aes(x = yday * 2 * pi / 365, y = all_mohk$y[1] * 0.9), color = "black", size = 0.2) +
  geom_line(aes(x = yday * 2 * pi / 365, y = all_mohk$y[1] * 1), color = "black", size = 0.2) + # inner and outer lines
  # Day length as a line####
  # geom_line(data=daylength_Mohonk,aes(x=yday*2*pi/365,y=DayLength),color="black",size=1)+
  ggnewscale::new_scale_fill() +
  ...
  theme(
    ...
    legend.margin = margin(3.0, 0, 0, 0, unit = "cm"),
    legend.background = element_rect(color = NA, fill = NA), # make the background of the legend box blank
    legend.key.size = unit(0.6, "line"), # increase the size of the legend points
    plot.margin = unit(c(-0.5, -0.5, -0.5, -0.2), "cm"), # spread out the plot a bit to minimize the white space
