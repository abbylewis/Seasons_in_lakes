...
(gg.clock.mohonk <- ggplot(data = all_mohk) +
  ...
  # Strat label
  geom_label(
    data = tibble(x = (c(228)) * 2 * pi / 365, y = y_limit_upper * 0.9),
...
(gg.clock.rere <- ggplot(data = all_rere) +
  ...
  # Strat label
  geom_label(
    data = tibble(x = (c(132)) * 2 * pi / 365, y = y_limit_upper * 0.9),
...
(gg.clock.wlb <- ggplot(data = all_wlb) +
  ...
  # Strat label
  geom_label(
    data = tibble(x = (c(290)) * 2 * pi / 365, y = y_limit_upper * 0.8),
...
ggplot(data = kin_strat, aes(x = Date, y = delta_density)) +
  geom_point()
...
kin_daylength <- tibble(Date = seq.POSIXt(as.POSIXct("1993-01-01 00:00:00", tz = "Asia/Jerusalem"), as.POSIXct("1993-12-31 00:00:00", tz = "Asia/Jerusalem"), by = "day")) %>%
  ...
  mutate(yday = yday(Date)) %>%
  mutate(Value = DayLength)
ggplot(data = kin_daylength, aes(x = yday, y = DayLength)) +
  geom_point()
...
ggplot(data = all_kin, aes(x = yday, y = delta_density)) +
  geom_point()
...
(gg.full <- wrap_plots(full.list, design = layout, heights = unit(c(2.7, 1.9, 2.7), c("in", "in", "in"))) +
