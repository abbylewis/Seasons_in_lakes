...
(gg.clock.mohonk <- ggplot(data = all_mohk) +
  ...
  # Daylength label
  geom_label(data = tibble(x = (c(130)) * 2 * pi / 365, y = y_limit_upper), aes(x = x, y = y), label = "Day length", size = 9 * (5 / 14)) + # The size= in element_text is 14/5 of the size= in geom_text
...
(gg.clock.rere <- ggplot(data = all_rere) +
  ...
  geom_segment(data = tibble(x = (c(355, 80, 172, 264)) * 2 * pi / 365, y = 3) %>% mutate(xend = x, y = y_limit_lower, yend = 3), aes(x = x, xend = xend, y = y, yend = yend), color = "black") +
  geom_rect(data = tibble(xmin = 0 * 2 * pi / 365, xmax = 365 * 2 * pi / 365, ymin = y_limit_lower, ymax = 1), aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), fill = "white", color = "white") +
  ...
  # Strat arrow
  geom_segment(data = tibble(x = (c(290)) * 2 * pi / 365, y = 3.5) %>% mutate(xend = x, yend = y_limit_upper), aes(x = x, xend = xend, y = y, yend = yend), color = "black") +

  # Strat label
  geom_label(
    data = tibble(x = (c(290)) * 2 * pi / 365, y = y_limit_upper),
    aes(x = x, y = y), label = "Stratification", size = 9 * (5 / 14),
    hjust = .8
...
gg.2panel <- wrap_plots(list(gg.clock.mohonk, gg.clock.rere), nrow = 1, guides = "collect") &
  theme(
    ...
    legend.box.margin = margin(-50, 0, -10, 0),
