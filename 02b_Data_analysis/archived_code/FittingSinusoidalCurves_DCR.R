...
sample_lats <- tibble(
  doy = rep(seq(1, 365), 19),
  latitude = rep(seq(0, 180, by = 10) - 90, each = 365)
) %>%
  mutate(daylength = geosphere::daylength(latitude, doy))

# plot them all####
ggplot(data = sample_lats, aes(x = doy, y = daylength, color = latitude)) +
  geom_point()

# plot a few select ones
ggplot(data = sample_lats %>% filter(abs(latitude) < 70), aes(x = doy, y = daylength, color = latitude)) +
  geom_point()
...
sum <- continuous_data2 %>%
  ...
  summarize(
    vertical = sinreg(yday, Value, fixed_period = 365.25)[[1]][[1]],
...
temp5 <- continuous_data2 %>%
  group_by(Lake, Variable) %>%
  do(lm.fit.model = lm(data = temp, Value ~ cos(2 * pi * yday / 365.25) + sin(2 * pi * yday / 365.25)))
...
ggplot(
  ...
  aes(x = yday, y = Value, color = abs(Latitude_DD), group = abs(Latitude_DD))
) +
  geom_line()
...
ggplot(
  ...
  aes(x = yday, y = Value, color = abs(Latitude_DD))
) +
  geom_point() +
...
temp <- continuous_data2 %>%
  filter(Lake == "Mohonk Lake (2017)" & Variable == "Day length") %>%
  distinct()
...
summary(fit.lm)
temp <- temp %>%
  mutate(predict_lm_Value = predict(fit.lm, newdata = list(yday = 1:366))) %>%
...
coeff <- coef(nls.mod)
f <- function(x, a, b, c) {
  a + b * sin(2 * pi * (1 / 365.25) * x + c)
}

# plot out the three different versions for proof of concept
ggplot(data = temp, aes(x = yday, y = Value)) +
  geom_point() +
  geom_line(aes(y = predictSin_Value), color = "blue") +
