...
for_plot <- continuous %>%
  ...
  summarize(
    p = wilcox.test(Value ~ Method)$p.value,
    p = case_when(
      p >= 0.05 ~ "n.s.",
      p < 0.05 & p >= 0.01 ~ "*",
      p < 0.01 & p >= 0.001 ~ "**",
...
for_plot %>%
  ...
  geom_jitter(aes(fill = abs(lats), shape = sig),
    width = 0.2,
  ...
  theme_bw(base_size = 9) +
  scale_shape_manual(
    name = "Significance of difference\nbetween definitions\nat each lake",
    values = c(21, 24)
  ) +
