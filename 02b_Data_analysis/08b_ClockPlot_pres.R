...
getDayLength <- function(date, latitude, longitude) {
  ...
  cos_omega <- (sin(-0.83 * 2 * pi / 360) -
    (sindelta * sin(latitude * 2 * pi / 360))) /
...
getDayLength(
  date = as.POSIXct("2017-03-21 00:00:00", tz = "EST"),
  latitude = 41.8, longitude = -74.2
)
...
daylength_Mohonk <- tibble(
  date = seq.POSIXt(as.POSIXct("2017-01-01 00:00:00", tz = "EST"),
    as.POSIXct("2017-12-31 00:00:00", tz = "EST"),
    by = "day"
  )
) %>%
  mutate(DayLength = getDayLength(date, latitude = 41.8, longitude = -74.2)) %>%
  mutate(yday = yday(date))
ggplot(data = daylength_Mohonk, aes(x = yday, y = DayLength)) +
  geom_point()
...
annual_data_mohk <- tibble(
  yday = seq(1, 365, by = 1),
  y = min(daylength_Mohonk$DayLength),
  y2 = max(daylength_Mohonk$DayLength)
)

# Mohonk ice####
# Join them all together, add Mohonk Ice####
all_mohk <- annual_data_mohk %>%
  left_join(., mohk_strat %>% mutate(delta_density = Value) %>%
    dplyr::select(yday, delta_density)) %>%
  left_join(., daylength_Mohonk %>% mutate(day_length = DayLength) %>%
    dplyr::select(yday, day_length)) %>%
  mutate(Ice = ifelse(yday <= 108, "ice", "open"), )
# Add on extra values to complete the circle####
all_mohk <- all_mohk %>%
  # add in a duplicate first row
  bind_rows(all_mohk %>% slice(1) %>% mutate(yday = 0), .) %>%
  # add in a duplicate last row
  bind_rows(., all_mohk %>% slice(365) %>% mutate(yday = 366))

# Fill in the missing data with 0.1
all_mohk <- all_mohk %>%
  # linearly interpolate
  mutate(delta_density = na.approx(delta_density)) %>%
  # get rid of negatives
  mutate(delta_density = ifelse(delta_density < 0, 0, delta_density))
...
tanganyika_cont <- continuous_data2 %>%
  filter(Lake == "Lake Tanganyika (2012-2013)") %>%
  mutate(yday2 = yday - 194)
...
ggplot(data = tanganyika_daylength, aes(x = yday, y = DayLength)) +
  geom_point()
...
(gg.clock.tanganyika <- ggplot(data = all_tanganyika) +
  ...
  # Month names
  geom_text(
    data = tibble(x = (c(1, 32, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335)) * 2 * pi / 365, label = month.abb),
  ...
  # Daylength arrow
  geom_segment(
    data = tibble(x = (c(75)) * 2 * pi / 365, y = 2.5) %>%
  ...
  # Daylength label
  geom_label(
    data = tibble(x = (c(75)) * 2 * pi / 365, y = y_limit_upper * .75),
    aes(x = x, y = y),
...
wlb_strat <- wlb_cont %>% mutate(
  Date = wlb_dates$day,
  delta_density = Value
)
ggplot(data = wlb_strat, aes(x = Date, y = delta_density)) +
  geom_point()
...
wlb_daylength <- tibble(Date = seq.POSIXt(as.POSIXct("2023-06-21 00:00:00", tz = "Pacific/Auckland"), as.POSIXct("2024-06-20 00:00:00", tz = "Pacific/Auckland"), by = "day")) %>%
  ...
  mutate(yday = yday(Date)) %>%
  mutate(Value = DayLength)
ggplot(data = wlb_daylength, aes(x = yday, y = DayLength)) +
  geom_point()
...
(gg.clock.wlb <- ggplot(data = all_wlb) +
  ...
  # Ice
  geom_point(
    data = all_wlb %>% filter(Ice == "ice"), aes(x = yday * 2 * pi / 365, y = (3 + 4) / 2),
    shape = 21, color = "black", fill = "black", size = 1.7
  ) + # put blue dots for ice days in the outer ring
  geom_point(
    data = all_wlb %>% filter(Ice == "ice"), aes(x = yday * 2 * pi / 365, y = (3 + 4) / 2),
  ...
  geom_text(
    data = tibble(
      x = (c(1, 32, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335)) * 2 * pi / 365,
      label = month.abb
    ),
    aes(x = x, y = 4.7, label = label),
    color = "black", size = 2.8
  ...
  # Ice arrow
  geom_segment(
    data = tibble(x = (c(259)) * 2 * pi / 365, y = 3.5) %>%
  ...
  # Ice label
  geom_label(
    data = tibble(x = (c(259)) * 2 * pi / 365, y = y_limit_upper * 0.75),
    aes(x = x, y = y), label = "Ice", size = 9 * (5 / 14),
    hjust = 1
  ) + # The size= in element_text is 14/5 of the size= in geom_text
...
(gg.worldMap.naturalEarth <- ggplot() +
  ...
  geom_point(
    data = tibble(
      Latitude = c(-77.720192, 41.77, -6.243461),
