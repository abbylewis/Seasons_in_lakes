...
#for iso3
countries_iso3 <- countries[!countries$iso3_lower%in%c("and", "can", "ago", "are", "per")]
...
detect_state <- function(text) {
  # Match full country name
  found = sum(stri_detect_regex(
    text, 
    paste0("\\b",tolower(state.name),"\\b")))
  if (found > 0) return("United States")
...
sum(!is.na(spat2$Country))/nrow(spat2) #Currently matching ~71% of abstracts
...
jpeg("../03a_Figures/Sunburst_by_country_comb.jpg", res = 300, width = 8, height = 5, units = "in")
ggpubr::ggarrange(og+
                  ...
                  counts + theme(legend.position = "none"),
                  widths = c(1,1.2),
                  common.legend = T,
                  legend = "bottom")
dev.off()
