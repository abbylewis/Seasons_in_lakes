...
if (!file.exists("../03a_Figures")) {
  dir.create("../03a_Figures")
}
...
select <- reference %>%
  filter(Lake_data %in%
    c(
      ...
      "Midway Pond (2023)", # 44.931
      "Beaverdam Reservoir (2021)", # 37.3164
      "Lake Tanganyika (2012-2013)", #-6.24346
      "Lake Batur (2023-2024)", #-8.259
      "Lake Rerewhakaaitu (2022-2023)", #-38.2936
      "West Lake Bonney (2023-2024)" #-77.720
    )) %>%
  arrange(-Latitude_DD)
...
```{r}
# Load discrete data
in_lake_csvs <- list.files(
  path = "../01b_Processed_data",
...
lake_relabel <- function(lakes) {
  labels <- c(
    "Arendsee (2020)" = "bold('a)')~`Arendsee (2020); Z`[max]~`= 50 m`",
    "Midway Pond (2023)" = "bold('b)')~`Midway Pond (2023); Z`[max]~`= 9 m`",
    "Beaverdam Reservoir (2021)" = "bold('c)')~`Beaverdam Reservoir (2021); Z`[max]~`= 11 m`",
    "Lake Tanganyika (2012-2013)" = "bold('d)')~`Lake Tanganyika (2012-2013); Z`[max]~`= 1470 m`",
    "Lake Batur (2023-2024)" = "bold('e)')~`Lake Batur (2023-2024); Z`[max]~`= 88 m`",
    "Lake Rerewhakaaitu (2022-2023)" = "bold('f)')~`Lake Rerewhakaaitu (2022-2023); Z`[max]~`= 15 m`",
    "West Lake Bonney (2023-2024)" = "bold('g)')~`West Lake Bonney (2023-2024); Z`[max]~`= 40 m`"
  )
...
discrete_ice <- temperate %>%
  ...
  scale_color_manual("Temperate\nseasons",
    values = c(
      "grey90", "grey77", "grey64", "grey51",
      "grey", "grey", "grey", "grey", "grey"
    ), # placeholders
    breaks = c("Winter", "Spring", "Summer", "Autumn")
  ) +
  scale_x_continuous(
    breaks = c(0, 90, 183, 276, 365),
    labels = c(
      "Winter\nsolstice", "Spring\nequinox",
      "Summer\nsolstice", "Autumn\nequinox", "Winter\nsolstice"
    )
  ...
  scale_color_manual("Stratification",
    values = c("#efbebe", "#DB8080"),
    breaks = c("Mixed", "Stratified")
  ) +
  guides(color = guide_legend(ncol = 1, title.position = "top")) +
  ggnewscale::new_scale_color() +
  geom_segment(
  ...
  scale_color_manual("Ice",
    values = c("#8AB4E3", "#E5EDF7"),
...
levels_cont <- c(
  "Surface_temperature", "AirTemp_C_Average", "Surface_chla",
...
continuous_data <- all %>%
  ...
  mutate(x = yday) %>%
  mutate(
    var_class = ifelse(Variable %in% c(
...
continuous <- continuous_data %>%
  ...
  scale_color_manual(values = c(
    "#6B7CB3", "#1F363D", "#8DD65C", "lightblue",
    "#FFC233", "#F05D5E"
  )) +
  geom_line(alpha = 0.2) +
  ...
  labs(
    y = "% of range          ",
    title = "Seasonally-varying parameters"
  ) +
...
```{r}
jpeg("../03a_Figures/Seasons_data_ice_reduced.jpg", width = 8.5, height = 8, units = "in", res = 300)
