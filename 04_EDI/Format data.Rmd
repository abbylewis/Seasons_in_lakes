...
in_lake_formatted_midway <- in_lake_data_surf %>%
  ...
  pivot_longer(
    cols = c(
      WaterTemperature_C, Chla_ugL, DO_mgL,
...
# Format continuous data
in_lake_formatted_rerewhakaaitu <- in_lake_raw %>%
  rename(
    Value = avg_value,
    Variable = variable,
    Depth_m = depth_rnd
...
# Format continuous data
met_formatted_rerewhakaaitu <- met %>%
  mutate(Date = as.Date(DateTime)) %>%
  mutate(
    variable = ifelse(variable == "TmpAir", "AirTemp_C", variable),
...
# Format continuous data
in_lake_formatted_mohonk <- all_data %>%
...
temp_format <- temp %>%
  ...
  mutate(
    Depth_m = str_extract(name, "[0-9]+_[0-9]+"),
...
chla_format <- chla %>%
  ...
  mutate(
    Depth_m = sub("Chla_", "", name),
    Depth_m = as.numeric(Depth_m)
  ) %>%
  group_by(Date, Depth_m) %>%
  summarise(Chla_ugL = mean(Chla_ugL, na.rm = T))

in_lake_formatted_erken <- temp_format %>%
  full_join(do_format) %>%
...
# Format continuous data
met_formatted_erken <- met %>%
  mutate(Date = as.Date(TMSTAMP)) %>%
  group_by(Date) %>%
  summarize(
    AirTemp_C = mean(Air_Temp_AS_Avg, na.rm = T),
...
inspect <- ice %>%
  mutate(
    TIME_ICE_ON = as.Date(TIME_ICE_ON),
...
ice_erken <- data.frame(
  start = c(0, inspect$start, inspect$end),
...
# Format continuous data
in_lake_formatted_arendsee <- in_lake_data_chla %>%
  full_join(in_lake_data_do) %>%
  ...
  mutate(
    Depth_m = as.numeric(str_extract(Depth_m, "[0-9]+")),
    Date = as.Date(time)
  ) %>%
  pivot_wider(names_from = c(var), values_from = Value) %>%
  group_by(Date, Depth_m) %>%
  summarize(
    WaterTemperature_C = mean(Temp_C, na.rm = T),
...
# Format continuous data
met_formatted_arendsee <- met %>%
  mutate(Date = as.Date(time)) %>%
  group_by(Date) %>%
  summarize(
    AirTemp_C = mean(AirT...C., na.rm = T),
...
# Load data
in_lake_data <- read_csv("../01a_Raw_data/Batur_water_2022-2024.csv") %>%
  mutate(Date = as.Date(datetime)) %>%
  filter(!is.na(Date))

# Format continuous data
in_lake_formatted_batur <- in_lake_data %>%
  pivot_longer(contains("wtr"),
    names_prefix = "wtr_",
  ...
  select(Lake, Date, Depth_m, WaterTemperature_C)

# Load data
met <- read_csv("../01a_Raw_data/Batur_met_2022-2024.csv")

# Format continuous data
met_formatted_batur <- met %>%
  mutate(Date = as.Date(datetime)) %>%
  group_by(Date) %>%
  summarize(
    airT = mean(airT, na.rm = T),
    Solar = mean(sw, na.rm = T)
  ) %>%
  rename(
    AirTemp_C = airT,
    ShortwaveRadiation_Wm2 = Solar
  ) %>%
  select(ShortwaveRadiation_Wm2, AirTemp_C, Date) %>%
  mutate(Lake = LAKE)

# No ice
solstice <- as.Date("2023-06-21")
ice_batur <- data.frame(
  Lake = LAKE,
  Season = "Open water",
  Method = "Ice",
  start = 0,
...
# Format continuous data
in_lake_formatted_tanganyika <- in_lake_data %>%
  select(Date, Temp, DO, Chla, Depth) %>%
  rename(
    Depth_m = Depth,
    WaterTemperature_C = Temp,
    DO_mgL = DO,
    Chla_ugL = Chla
  ) %>%
  mutate(Lake = LAKE) %>%
  select(Lake, Date, Depth_m, WaterTemperature_C, DO_mgL, Chla_ugL)

# Load data
met <- read_csv("../01a_Raw_data/DATA_Weather_forSeasonsMS_v1.csv")

# Format continuous data
met_formatted_tanganyika <- met %>%
  mutate(Date = as.Date(Date)) %>%
  rename(
    AirTemp_C = Temp_mean,
    ShortwaveRadiation_Wm2 = PAR_mean
  ) %>%
  select(ShortwaveRadiation_Wm2, AirTemp_C, Date) %>%
  mutate(Lake = LAKE)
...
```{r}
# Met
met_all <- met_formatted_midway %>%
  bind_rows(met_formatted_rerewhakaaitu) %>%
  bind_rows(met_formatted_mohonk) %>%
  bind_rows(met_formatted_erken) %>%
  bind_rows(met_formatted_arendsee) %>%
  bind_rows(met_formatted_batur) %>%
  bind_rows(met_formatted_tanganyika) %>%
  select(
    Lake, Date,
...
# Ice
ice_all <- ice_mi %>%
  bind_rows(ice_rerewhakaaitu) %>%
  bind_rows(ice_mohonk) %>%
  bind_rows(ice_erken) %>%
  bind_rows(ice_arendsee) %>%
  bind_rows(ice_batur) %>%
  bind_rows(ice_tanganyika)

format_ice <- function(start, end, Season, Lake, solstice) {
  out <- data.frame(
    Date = as.Date(solstice + start):as.Date(solstice + end),
...
# In lake
in_lake_all <- in_lake_formatted_midway %>%
  bind_rows(in_lake_formatted_rerewhakaaitu) %>%
  bind_rows(in_lake_formatted_mohonk) %>%
  bind_rows(in_lake_formatted_erken) %>%
  bind_rows(in_lake_formatted_arendsee) %>%
  bind_rows(in_lake_formatted_tanganyika) %>%
  bind_rows(in_lake_formatted_batur) %>%
