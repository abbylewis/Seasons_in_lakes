...
catwalk_format <- catwalk %>%
  ...
  filter(Year == 2021) %>%
  group_by(Date) %>%
  summarize(Surface_temperature = mean(EXOTemp_C_1, na.rm = T),
            Surface_chla = mean(EXOChla_ugL_1, na.rm = T)) %>%
...
fcr_dens <- catwalk_strat |> 
  mutate(Diff_C_surf_9 = wtr_00.1 - wtr_09.0,
         Diff_Dens_surf_bot = water.density(wtr_09.0) - water.density(wtr_00.1),
         Strat_1C_surf_9 = ifelse(Diff_C_surf_9 < 1, "Not_Strat", "Strat"),
         Strat_Dens0.1_surf_9 = ifelse(Diff_Dens_surf_bot > 0.1, "Strat", "Not_Strat"))
...
strat_export <- strat_metrics_long %>%
  ...
  full_join(data.frame(Variable = "Diff_Dens_surf_bot", yday = 0, Strat = "Mixed")) %>%
  arrange(yday) %>%
...
write.csv(strat_export, "../01b_Processed_data/FCR_stratification_discrete.csv", row.names = F)
write.csv(strat_metrics_long, "../01b_Processed_data/FCR_stratification_continuous.csv", row.names = F)
```
...
write.csv(ice_format2, "../01b_Processed_data/FCR_ice.csv", row.names = F)
```
