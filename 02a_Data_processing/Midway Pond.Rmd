...
```{r}
# Load data
in_lake_formatted <- read_csv("https://pasta-s.lternet.edu/package/data/eml/edi/1312/2/f8e0e7b22826772424f156b9565d1498") %>%
  filter(Lake == "Midway Pond") %>%
  mutate(Date = as.Date(Date) + offset) %>%
  filter(year(Date) == 2023) %>%
  select(-Chla_fluorescence) %>%
  pivot_wider(names_from = Layer, values_from = c(WaterTemperature_C, DO_mgL, Chla_ugL)) %>%
  rename(Surface_temperature = WaterTemperature_C_Surface,
         Bottom_temperature = WaterTemperature_C_Bottom,
         Surface_chla = Chla_ugL_Surface,
         Surface_DO = DO_mgL_Surface,
         Bottom_DO = DO_mgL_Bottom) %>%
