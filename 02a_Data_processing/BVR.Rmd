...
catwalk_format <- catwalk %>%
  mutate(
    Date = as.Date(DateTime) + offset,
    Year = year(Date)
  ) %>%
  filter(Year == 2021) %>%
  group_by(Date) %>%
  summarize(
    Surface_temperature = mean(EXOTemp_C_1.5, na.rm = T),
  ...
  pivot_longer(
    cols = c(Surface_temperature, Surface_chla, Bottom_DO, Turbidity, Surface_DO),
...
catwalk_strat <- read.csv("../01b_Processed_data/BVR_longoutput.csv") %>%
  ...
  summarize(
    Diff_Dens_surf_bot = water.density(Reading[which.max(Sensor_depth)]) -
...
ice_format2 <- ice_format %>%
  mutate(
    group = rep(1:(nrow(ice_format) / 2), each = 2),
...
ice_format3 <- ice_format %>%
  ...
  arrange(yday) %>%
  mutate(
    group = rep(1:((nrow(ice_format) + 2) / 2), each = 2),
