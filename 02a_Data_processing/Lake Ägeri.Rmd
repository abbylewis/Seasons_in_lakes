...
read_ncdf_lake <- function(file) {
  ...
  combined_df <- data.frame(
    DateTime = rep(time, length(depth)),
...
in_lake_formatted <- in_lake_data %>%
  ...
  group_by(Date) %>%
  summarise(
    Surface_temperature = mean(Temp_C[Depth_m == 2], na.rm = T),
  ...
  pivot_longer(
    cols = c(Surface_temperature, Surface_chla, Bottom_DO, Surface_DO),
...
met_formatted <- met %>%
  ...
  group_by(Date) %>%
  summarize(
    AirTemp_C_Average = mean(Temp_C, na.rm = T),
