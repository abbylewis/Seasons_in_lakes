...
in_lake_formatted <- in_lake_data %>%
  ...
  mutate(Layer = case_when(
    name == "stage_watertemp_degc" ~ "Surface",
...
strat_sum <- in_lake_formatted %>%
  ...
  mutate(Bottom_sal = 130, Surface_sal = 0) %>%
  mutate(
    Diff_Dens_surf_bot = water.density(Bottom_temperature, Bottom_sal) -
...
met_formatted <- met %>%
  ...
  mutate(
    Date = as.Date(date_time) + offset,
    doy = yday(Date)
  ) %>%
  filter(year(Date) == 2023) %>%
  rename(
    AirTemp_C_Average = airtemp_1m_degc,
...
ice_formatted <- tibble(Lake = LAKE, Season = "Ice", Method = "Ice", start = 0, end = 365)
