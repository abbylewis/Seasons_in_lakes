...
in_lake_formatted <- in_lake_data %>%
  rename(
    Surface_DO = oxygenDissolvedPercentOfSaturation_pct_1p5m,
    Surface_temperature = waterTemperature_degC_0p5m
  ) %>%
  pivot_longer(
    cols = c(Surface_temperature, Surface_DO),
...
strat_sum <- in_lake_data %>%
  group_by(yday) %>%
  summarize(
    Diff_Dens_surf_bot = water.density(waterTemperature_degC_13p5m) -
...
met_formatted <- met_2007 %>%
  ...
  filter(year(Date) == 2008) %>%
  rename(
    AirTemp_C_Average = airTemperature_degC,
    Shortwave = radiationIncomingPAR_umolm2s
  ) %>%
  pivot_longer(
    cols = c(AirTemp_C_Average, Shortwave),
    names_to = "Variable", values_to = "Value"
  ) %>%
